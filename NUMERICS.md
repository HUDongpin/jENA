...
Function-valued `weightBy` is applied once per windowed co-occurrence cell, BEFORE unit accumulation, receiving a single-element array. This matches rENA's runtime behavior exactly (golden-verified with a `sqrt` weighting on both datasets): rENA's implementation applies `weight.by` with `by = 1:nrow(...)`, so the function sees each row's cell as a length-1 vector — despite its docstring saying the function is applied "after accumulation" (a documentation bug, noted in [rENA#48](https://gitlab.com/epistemic-analytics/qe-packages/rENA/-/issues/48)). Consequently rENA's `weight.by = sum` is the identity on raw window products, which is what jena's `"sum"` string does. Cross-window aggregation semantics (e.g. `max` over a unit's windows) are deliberately NOT what `weightBy` means; if ever needed, that belongs to a new explicitly-named option.
